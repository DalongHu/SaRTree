...
my $loc = shift;
my $name = shift;
unless (defined $raw and defined $out and defined $snp and defined $rec and defined $indel and defined $loc and defined $name){
	print "Usage: perl $0 <rawTree(nwk)> <out(branch sum)> <snpList> <recList> <indelList> <location file(from StrainLocater)> <namelist>\n2017-06-30\n";
	exit;
}
...
print SUM "Branch\tPattern\tStrain\tType\tBootstrap(%)\tSNPs\tRecombinations\tIndels\tDescentants\n";
...
foreach my $type(sort {$branch{$a} <=> $branch{$b}} keys %branch){
	...
	print SUM "$oBranch\t$oPattern\t$oStrain\t$oType\t$oBoot\t$oSnp\t$oRec\t$oIndel\t$oDescendants\n";
}
