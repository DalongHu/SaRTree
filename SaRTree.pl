...
==============Options==============
	-step/-n <string> optional, \"REC\" or \"SNP\" to stop the pipeline after done RecDetect or SNP calling step\n 
...
if ($model eq 'raw'){
	...
	foreach my $sample(@samples){
		...
		$cmd .= "$mauveBin $mauveConfig --output=$dir/mauve/$name.mauve $ref.$i_ref.fna $sample"; # Fixed, support for HPC
		if(defined $thread){
			$cmd .= ' & ';
			$count_thread++;
